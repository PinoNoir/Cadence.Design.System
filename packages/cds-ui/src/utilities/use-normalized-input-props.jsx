...
import { formRequirement } from '../components/text-input/TextInput.css';
 ...
 */
export function useNormalizedInputProps({ id, readOnly, disabled, invalid, invalidText, warn, warnText }) {
  ...
  if (normalizedProps.invalid) {
    normalizedProps.icon = <Icon icon="mdi:alert-rhombus" focusable="false" color="#E12339" />;
    normalizedProps.validation = (
      <div className={`${formRequirement}`} id={normalizedProps.invalidId}>
        {invalidText}
      </div>
    );
  } else if (normalizedProps.warn) {
    normalizedProps.icon = <Icon icon="mdi:alert" focusable="false" color="#BC881A" />;
    normalizedProps.validation = (
      <div className={`${formRequirement}`} id={normalizedProps.warnId}>
        {warnText}
      </div>
    );
  }
